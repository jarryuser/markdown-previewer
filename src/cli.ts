...
// Excludes ambiguous symbols like ⛶ (U+26F6) that render as 1-column text.
function isBmpEmojiPresentation(cp: number): boolean {
  return (
    ...
    (cp >= 0x2648 && cp <= 0x2653) ||
    (cp >= 0x265F && cp <= 0x2660) ||
    cp === 0x2663 || (cp >= 0x2665 && cp <= 0x2666) ||
    cp === 0x2668 || cp === 0x267B ||
    (cp >= 0x267E && cp <= 0x267F) ||
    cp === 0x2693 || (cp >= 0x2694 && cp <= 0x2697) ||
    cp === 0x2699 || (cp >= 0x269B && cp <= 0x269C) ||
    (cp >= 0x26A0 && cp <= 0x26A1) ||
    cp === 0x26A7 || (cp >= 0x26AA && cp <= 0x26AB) ||
    (cp >= 0x26B0 && cp <= 0x26B1) ||
    (cp >= 0x26BD && cp <= 0x26BE) ||
    (cp >= 0x26C4 && cp <= 0x26C5) ||
    (cp >= 0x26CE && cp <= 0x26CF) ||
    cp === 0x26D1 || (cp >= 0x26D3 && cp <= 0x26D4) ||
    (cp >= 0x26E9 && cp <= 0x26EA) ||
    (cp >= 0x26F0 && cp <= 0x26F5) ||
    (cp >= 0x26F7 && cp <= 0x26FA) ||
    cp === 0x26FD ||
    cp === 0x2702 || cp === 0x2705 ||
    (cp >= 0x2708 && cp <= 0x270D) ||
    cp === 0x2712 || cp === 0x2714 || cp === 0x2716 ||
    cp === 0x271D || cp === 0x2721 || cp === 0x2728 ||
    (cp >= 0x2733 && cp <= 0x2734) ||
    cp === 0x2744 || cp === 0x2747 ||
    cp === 0x274C || cp === 0x274E ||
    (cp >= 0x2753 && cp <= 0x2755) ||
    cp === 0x2757 || (cp >= 0x2763 && cp <= 0x2764) ||
    (cp >= 0x2795 && cp <= 0x2797) ||
...
function isWideChar(cp: number): boolean {
  return (
    cp > 0xFFFF ||                               // supplementary (emoji, etc.)
    (cp >= 0x1100 && cp <= 0x115F) ||            // Hangul Jamo
    isBmpEmojiPresentation(cp) ||                // BMP emoji with Emoji_Presentation=Yes
...
if (isTerminal) {
  ...
  screen.key(['g', 'home'],     () => { (pager as any).scrollTo(0); screen.realloc(); screen.render(); });
  screen.key(['S-g', 'end'],    () => { (pager as any).scrollTo((pager as any).getScrollHeight()); screen.realloc(); screen.render(); });
